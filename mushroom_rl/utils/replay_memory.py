...
class ReplayMemory(Serializable):
    ...
    def __init__(self, initial_size, max_size):
        ...
        # note replay memory has been modified as due to the possibility of conducting additional search some more things
        # have to be stored
        self._add_save_attr(
            ...
            _rewards='pickle!',
            _next_states='pickle!',
            _explored_act_hist='pickle!',
            _explored_next_state='pickle!',
            _explored_sing_step_r='pickle!',
            _explored_next_states_absorbing='pickle!',
            _explored_next_states_value='pickle!',
            _absorbing='pickle!',
            _last='pickle!'
        )
    ...
    def add(self, dataset):
        ...
        for i in range(len(dataset)):
            # at first: remove all the elements that have been at this index before
            abc = self._states[self._idx]
            del abc
            abc = self._actions[self._idx]
            del abc
            abc = self._rewards[self._idx]
            del abc
            abc = self._next_states[self._idx]
            del abc
            abc = self._explored_act_hist[self._idx]
            del abc
            abc = self._explored_next_state[self._idx]
            del abc
            abc = self._explored_sing_step_r[self._idx]
            del abc
            abc = self._explored_next_states_absorbing[self._idx]
            del abc
            abc = self._explored_next_states_value[self._idx]
            del abc
            abc = self._absorbing[self._idx]
            del abc
            abc = self._last[self._idx]
            del abc
            # now fill it in
            ...
            self._explored_act_hist[self._idx] = dataset[i][4]
            self._explored_next_state[self._idx] = dataset[i][5]
            self._explored_sing_step_r[self._idx] = dataset[i][6]
            self._explored_next_states_absorbing[self._idx] = dataset[i][7]
            self._explored_next_states_value[self._idx] = dataset[i][8]

            self._absorbing[self._idx] = dataset[i][9]
            self._last[self._idx] = dataset[i][10]
    ...
    def get(self, n_samples):
        ...
        for i in np.random.randint(self.size, size=n_samples):
            ...
            ea.append(self._explored_act_hist[i])
            en.append(self._explored_next_state[i])
            er.append(self._explored_sing_step_r[i])
            ena.append(self._explored_next_states_absorbing[i])
            env.append(self._explored_next_states_value[i])
            ab.append(self._absorbing[i])
            last.append(self._last[i])

        return np.array(s), np.array(a), np.array(r), np.array(ss),\
            np.array(ea), np.array(en), np.array(er), np.array(ena), np.array(env), np.array(ab), np.array(last)
    ...
    def reset(self):
        ...
        self._explored_act_hist = [None for _ in range(self._max_size)]
        self._explored_next_state = [None for _ in range(self._max_size)]
        self._explored_sing_step_r = [None for _ in range(self._max_size)]
        self._explored_next_states_absorbing = [None for _ in range(self._max_size)]
        self._explored_next_states_value = [None for _ in range(self._max_size)]
