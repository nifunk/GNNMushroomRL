...
class TorchApproximator(Serializable):
    ...
    def predict(self, *args, output_tensor=False, **kwargs):
        ...
        if not self._use_cuda:
            ...
            torch_args = [torch.from_numpy(x).float().cuda()
        ...
        if check_loss:
            with tqdm(total=n_epochs if n_epochs < np.inf else None,
                ...
                while patience_count < patience and epochs_count < n_epochs:
                    ...
                    if len(val_args[0]):
                        ...
                        self._loglist["q_loss"].append(loss)
                    else:
            ...
            with trange(n_epochs, disable=self._quiet) as t_epochs:
                for _ in t_epochs:
                    mean_loss_current = self._fit_epoch(train_args, use_weights,
                                                        kwargs)
                    self._loglist["q_loss"].append(mean_loss_current)
    ...
    def _compute_batch_loss(self, batch, use_weights, kwargs):
        if use_weights:
            weights = torch.from_numpy(batch[-1]).float()#type(torch.float)
            if self._use_cuda:
                weights = weights.cuda()
            batch = batch[:-1]

        if not self._use_cuda:
            torch_args = [torch.from_numpy(x).float() for x in batch]
        else:
            torch_args = [torch.from_numpy(x).float().cuda() for x in batch]
    ...
    def diff(self, *args, **kwargs):
        ...
        """
        if not self._use_cuda:
            torch_args = [torch.from_numpy(np.atleast_2d(x)).float() for x in args]
        else:
            torch_args = [torch.from_numpy(np.atleast_2d(x)).float().cuda()
                          for x in args]
